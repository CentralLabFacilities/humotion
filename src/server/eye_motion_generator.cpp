...
//! \param target angle
//! \param current angle
void EyeMotionGenerator::setup_eyemotion(int dof, float target, float current_position, float current_velocity,
	...
	reflexxes_set_input(dof, target, current_position, current_velocity, timestamp, max_velocity, max_accel);
}
...
void EyeMotionGenerator::calculate_targets() {
	...
	if (config->use_neck_target_instead_of_position_eye) {
		...
		neck_pan_now = joint_interface_->get_ts_position(JointInterface::ID_NECK_PAN).get_newest_value();
		neck_tilt_now = joint_interface_->get_ts_position(JointInterface::ID_NECK_TILT).get_newest_value();
	}

	// calculate target angles for the eyes
	// right eye is dominant -> direct output
	float eye_pan_r_target = (requested_gaze_state_.pan + requested_gaze_state_.vergence / 2.0) - (neck_pan_now);
	...
	float eye_pan_l_target = (requested_gaze_state_.pan - requested_gaze_state_.vergence / 2.0) - (neck_pan_now);
	...
	humotion::Timestamp eye_pan_l_ts =
	   get_timestamped_state(JointInterface::ID_EYES_LEFT_LR, &eye_pan_l_now, &eye_pan_l_speed);

	humotion::Timestamp eye_pan_r_ts =
	   get_timestamped_state(JointInterface::ID_EYES_RIGHT_LR, &eye_pan_r_now, &eye_pan_r_speed);

	humotion::Timestamp eye_tilt_ts = get_timestamped_state(JointInterface::ID_EYES_BOTH_UD, &eye_tilt_now, &eye_tilt_speed);
	...
	joint_interface_->set_target(JointInterface::ID_EYES_LEFT_LR, reflexxes_position_output->NewPositionVector->VecData[0],
	                             reflexxes_position_output->NewVelocityVector->VecData[0]);
	joint_interface_->set_target(JointInterface::ID_EYES_RIGHT_LR, reflexxes_position_output->NewPositionVector->VecData[1],
	                             reflexxes_position_output->NewVelocityVector->VecData[1]);
	joint_interface_->set_target(JointInterface::ID_EYES_BOTH_UD, reflexxes_position_output->NewPositionVector->VecData[2],
