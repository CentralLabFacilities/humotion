...
WARNING :
   ...
   //! constructor
   MiddlewareRSB::MiddlewareRSB(string scope, Controller* c)
   : Middleware(scope, c) {
	...
	mouth_target_listener->addHandler(HandlerPtr(
	   new DataFunctionHandler<rst::robot::MouthTarget>(boost::bind(&MiddlewareRSB::incoming_mouth_target, this, _1))));

	gaze_target_listener = factory.createListener(base_scope + "/humotion/gaze/target");
	gaze_target_listener->addHandler(
	   HandlerPtr(new EventFunctionHandler(boost::bind(&MiddlewareRSB::incoming_gaze_target, this, _1))));
