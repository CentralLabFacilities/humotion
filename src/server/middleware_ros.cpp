...
MiddlewareROS::MiddlewareROS(const std::string& scope, Controller* c) : Middleware(scope, c) {
	...
	// set up subscribers
	mouth_target_subscriber_ =
	   nh_->subscribe("mouth/target", 150, &MiddlewareROS::incoming_mouth_target, this, ros::TransportHints().unreliable());
	gaze_target_subscriber_ =
	   nh_->subscribe("gaze/target", 150, &MiddlewareROS::incoming_gaze_target, this, ros::TransportHints().unreliable());
...
void MiddlewareROS::dynamic_reconfigure_callback(const humotion::humotionConfig& dyn_config, uint32_t level) {
	// fetch config
...
void MiddlewareROS::attach_to_reconfiguration_server(const ros::NodeHandle& priv_nodehandle) {
	...
	reconf_server_->setCallback(boost::bind(&MiddlewareROS::dynamic_reconfigure_callback, this, _1, _2));
}
