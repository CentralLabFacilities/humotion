...
float NeckMotionGenerator::get_breath_offset() {
	...
	if (breath_time_normalized <= 0.5) {
		...
		// deaccelerate
		breath_offset = config->breath_amplitude * (1.0 - 2.0 * pow(1.0 - breath_time_normalized, 2));
	}
...
void NeckMotionGenerator::calculate_targets() {
	...
	humotion::Timestamp neck_pan_ts = get_timestamped_state(JointInterface::ID_NECK_PAN, &neck_pan_now, &neck_pan_speed);

	humotion::Timestamp neck_tilt_ts = get_timestamped_state(JointInterface::ID_NECK_TILT, &neck_tilt_now, &neck_tilt_speed);

	humotion::Timestamp neck_roll_ts = get_timestamped_state(JointInterface::ID_NECK_ROLL, &neck_roll_now, &neck_roll_speed);
	...
	joint_interface_->set_target(JointInterface::ID_NECK_PAN, reflexxes_position_output->NewPositionVector->VecData[0],
	                             reflexxes_position_output->NewVelocityVector->VecData[0]);

	joint_interface_->set_target(JointInterface::ID_NECK_TILT, reflexxes_position_output->NewPositionVector->VecData[1],
	                             reflexxes_position_output->NewVelocityVector->VecData[1]);

	joint_interface_->set_target(JointInterface::ID_NECK_ROLL, reflexxes_position_output->NewPositionVector->VecData[2],
	...
	/*printf("\n%f %f %f %f %f #GAZELOG\n",
	   ...
	   get_current_position(JointInterface::ID_EYES_RIGHT_LR))/2.0, get_current_position(JointInterface::ID_NECK_PAN));*/
...
//! \param target angle
//! \param current angle
void NeckMotionGenerator::setup_neckmotion(int dof, float target, float current_position, float current_velocity,
