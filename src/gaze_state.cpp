...
void GazeState::dump() {
	...
	printf("> GAZE STATE: %4.2f [%2.1f] %4.2f [%2.1f] %4.2f [%2.1f] (PTR) vergence=%4.2f "
	       ...
	       pan, pan_offset, tilt, tilt_offset, roll, roll_offset, vergence, eyelid_opening_upper, eyelid_opening_lower,
	       eyebrow_left, eyebrow_right,
	       (gaze_type == GAZETYPE_ABSOLUTE ?
	           "absolute" :
	           (gaze_type == GAZETYPE_RELATIVE ? "relative" : (gaze_type == GAZETYPE_OVERRIDE ? "override" : "!INVALID!"))),
	       timestamp.to_seconds(), eyeblink_request_left, eyeblink_request_right);
}
